...
export function keepYourHandsOffYourFace(keypoints, minConfidence) {
  ...
  const lWristY = keypoints[9].position.y;
  if (lWristVisible) {
    if (lWristY <= shoulderY) beep();
  }

  // right Hand in your face?
  const rWristVisible = keypoints[10].score > minConfidence;
  const rWristY = keypoints[10].position.y;
  if (rWristVisible) {
...
const beep = (function () {
  ...
  return function () {
    ...
    osc.type = 'sawtooth';
    osc.frequency.setValueAtTime(800, ctx.currentTime); // value in hertz
    osc.detune.setValueAtTime(200, ctx.currentTime); // value in cents
